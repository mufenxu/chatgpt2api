...
def _public_task(task: dict[str, Any], *, include_urls: bool = True) -> dict[str, Any]:
    item = {
        "id": task.get("id"),
    ...
    if task.get("data") is not None:
        data = task.get("data")
        if include_urls or not isinstance(data, list):
            item["data"] = data
        else:
            item["data"] = [
    ...
    def submit_generation(
        ...
        *,
        ...
        payload = {
            ...
            "size": size,
            "quality": quality,
            "response_format": "b64_json" if identity.get("role") == "user" else "url",
            "base_url": base_url,
        }
        ...
        with self._lock:
            ...
            for task_id in requested_ids:
                ...
                if task is None and is_admin:
                    task = next((candidate for candidate in self._tasks.values() if candidate.get("id") == task_id), None)
                if task is None:
                    missing_ids.append(task_id)
                else:
                    items.append(_public_task(task, include_urls=is_admin))
            if not requested_ids:
                items = [
                    _public_task(task, include_urls=is_admin)
                    for task in self._tasks.values()
                    if is_admin or task.get("owner_id") == owner
                ]
        ...
        *,
        ...
        with self._lock:
            ...
            if task is not None:
                if cleaned:
                    self._save_locked()
                return _public_task(task, include_urls=identity.get("role") == "admin")
            quota_consumed = False
            if identity.get("role") == "user":
                self.quota_service.consume_quota(identity, 1)
                quota_consumed = True
            task = {
                "id": task_id,
                "owner_id": owner,
            ...
            try:
                ...
                if quota_consumed:
                    self.quota_service.refund_quota(identity, 1)
                raise
            should_start = True
