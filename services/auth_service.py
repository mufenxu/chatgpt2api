...
class AuthService:
    ...
    @staticmethod
    def _quota_value(value: object, default: int = 0) -> int:
        try:
    ...
    @staticmethod
    def _unlimited_quota() -> dict[str, object]:
        return {
            "unlimited": True,
            "quota_total": None,
            "quota_used": 0,
    ...
    def get_quota(self, identity: dict[str, object]) -> dict[str, object]:
        ...
        with self._lock:
            ...
            return {
                ...
                "quota_updated_at": public.get("quota_updated_at"),
            }

    def consume_quota(self, identity: dict[str, object], amount: int = 1) -> dict[str, object]:
        if identity.get("role") == "admin":
            return self._unlimited_quota()
        charge = max(1, int(amount))
        identity_id = self._clean(identity.get("id"))
        with self._lock:
            self._reload_locked()
            for index, item in enumerate(self._items):
                ...
                if quota_total - quota_used < charge:
                    raise QuotaExceededError("剩余额度不足，请联系管理员充值")
                next_item = dict(item)
                next_item["quota_used"] = quota_used + charge
                next_item["quota_updated_at"] = _now_iso()
                self._items[index] = next_item
                self._save()
        ...
        raise QuotaExceededError("用户不存在或已被禁用")
