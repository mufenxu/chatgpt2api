...
function ImagePageContent({ isAdmin }: { isAdmin: boolean }) {
  ...
  const parsedCount = useMemo(
    () => (isAdmin ? Number(clampImageCount(imageCount)) : 1),
    [imageCount, isAdmin],
  );
  ...
  const loadQuota = useCallback(async () => {
    if (!isAdmin) {
      try {
        ...
        setAvailableQuota(data.unlimited ? "不限" : String(data.quota_remaining ?? 0));
      } catch {
  ...
  return (
    <>
      <section className="mx-auto grid h-[calc(100dvh-4.75rem)] min-h-0 w-full max-w-[1380px] grid-cols-1 gap-2 overflow-hidden px-0 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] sm:h-[calc(100dvh-5.25rem)] sm:gap-3 sm:px-3 sm:pb-6 lg:grid-cols-[240px_minmax(0,1fr)]">
