...
from services.auth_service import AuthService, QuotaExceededError
...
class AuthServiceTests(unittest.TestCase):
    def test_user_quota_consume_and_reset(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            service = AuthService(JSONStorageBackend(Path(tmp_dir) / "accounts.json", Path(tmp_dir) / "auth_keys.json"))
            item, raw_key = service.create_key(role="user", name="Alice", quota_total=2)
            identity = service.authenticate(raw_key)

            self.assertIsNotNone(identity)
            self.assertEqual(service.get_quota(identity)["quota_remaining"], 2)
            self.assertEqual(service.consume_quota(identity)["quota_remaining"], 1)
            self.assertEqual(service.consume_quota(identity)["quota_remaining"], 0)
            with self.assertRaisesRegex(QuotaExceededError, "剩余额度不足"):
                service.consume_quota(identity)

            updated = service.update_key(item["id"], {"reset_quota_used": True}, role="user")
            self.assertIsNotNone(updated)
            self.assertEqual(updated["quota_remaining"], 2)
