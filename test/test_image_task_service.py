...
class FakeQuotaService:
    ...
    def consume_quota(self, _identity, _amount=1):
        self.consume_calls += 1

    def refund_quota(self, _identity, _amount=1):
        self.refund_calls += 1
...
class ImageTaskServiceTests(unittest.TestCase):
    def make_service(self, path: Path, handler=None, quota_service=None) -> ImageTaskService:
        return ImageTaskService(
            ...
            **({"quota_service": quota_service} if quota_service is not None else {}),
        )

    def test_duplicate_user_submit_consumes_quota_once(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            quota_service = FakeQuotaService()
            service = self.make_service(Path(tmp_dir) / "image_tasks.json", quota_service=quota_service)

            first = service.submit_generation(
                OTHER_OWNER,
                client_task_id="user-task-1",
                prompt="cat",
                model="gpt-image-2",
                size=None,
            )
            ...
            self.assertEqual(quota_service.consume_calls, 1)
